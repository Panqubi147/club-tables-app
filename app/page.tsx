...
const TABLES: TableConfig[] = [
  { number: 1, type: "pool", label: "Pool", className: "left-[10%] top-[16%]", shape: "vertical" },
  { number: 2, type: "pool", label: "Pool", className: "left-[10%] top-[45%]", shape: "vertical" },
  { number: 3, type: "pool", label: "Pool", className: "left-[10%] top-[73%]", shape: "vertical" },

  { number: 4, type: "heyball", label: "Heyball", className: "left-[38%] top-[9%]", shape: "horizontal" },
  { number: 5, type: "heyball", label: "Heyball", className: "left-[38%] top-[45%]", shape: "horizontal" },
  { number: 6, type: "heyball", label: "Heyball", className: "left-[38%] top-[77%]", shape: "horizontal" },

  { number: 7, type: "snooker", label: "Snooker", className: "left-[65%] top-[8%]", shape: "horizontal" },
  { number: 8, type: "snooker", label: "Snooker", className: "left-[65%] top-[32%]", shape: "horizontal" },
  { number: 9, type: "snooker", label: "Snooker", className: "left-[65%] top-[55%]", shape: "horizontal" },
  { number: 10, type: "snooker", label: "Snooker", className: "left-[65%] top-[78%]", shape: "horizontal" },
];
...
function getCurrentDayType(): DayType {
  ...
  return day === 0 || day >= 5 ? "weekend" : "week";
}
...
function isSameLocalDay(dateValue: string | null, selectedDate: string) {
...
export default function Home() {
  ...
  const [archiveDate, setArchiveDate] = useState(getTodayInputValue());
  ...
  const todayRevenue = history
    .filter((session) => isSameLocalDay(session.end_time, getTodayInputValue()))
    .reduce((sum, session) => sum + Number(session.final_price ?? 0), 0);

  const archiveSessions = history.filter((session) =>
    isSameLocalDay(session.end_time, archiveDate)
  );

  const archiveRevenue = archiveSessions.reduce(
    (sum, session) => sum + Number(session.final_price ?? 0),
    0
  );

  const latestGames = history.slice(0, 20);
      ...
      <div className="mx-auto max-w-7xl">
        <div className="mb-6 flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            <div className="rounded-2xl bg-white p-4 text-slate-900 shadow">
              <p className="text-sm text-slate-500">Dzisiejszy typ dnia</p>
              <p className="text-xl font-bold">
                {dayType === "week" ? "Tydzień" : "Weekend"} — {PRICES[dayType]} zł/h
              </p>
            </div>
        ...
        <div className="grid gap-6 lg:grid-cols-[1.4fr_0.9fr]">
          <section className="rounded-2xl bg-white p-4 text-slate-900 shadow md:p-6">
            ...
            <div className="relative mx-auto aspect-square max-w-3xl overflow-hidden rounded-xl border-4 border-slate-800 bg-zinc-400 shadow-inner">
              <div className="absolute left-[2%] top-[2%] text-2xl font-black uppercase tracking-wide text-black md:text-4xl">
                Mapa klubu
              </div>

              <div className="absolute left-[3%] top-[36%] -rotate-90 text-3xl font-black tracking-widest text-blue-700 md:text-5xl">
                POOLBILLARD
              </div>

              <div className="absolute left-[31%] top-[39%] -rotate-90 text-3xl font-black tracking-widest text-emerald-900 md:text-5xl">
                HEYBALL
              </div>

              <div className="absolute right-[0%] top-[38%] -rotate-90 text-3xl font-black tracking-widest text-green-700 md:text-5xl">
                SNOOKER
              </div>

              <div className="absolute bottom-[2%] right-[2%] text-3xl font-black text-slate-950 md:text-5xl">
          ...
          <section className="rounded-2xl bg-white p-4 text-slate-900 shadow md:p-6">
            ...
            {!selectedTable || !selectedConfig ? (
              ...
              <div className="space-y-4">
                ...
                <div className="rounded-xl bg-slate-100 p-4">
                  <p className="text-sm text-slate-500">Automatyczna stawka</p>
                  <p className="text-xl font-bold">
                    {dayType === "week" ? "Tydzień" : "Weekend"} — {PRICES[dayType]} zł/h
                  </p>
                  <p className="mt-1 text-sm text-slate-500">
                    Poniedziałek–czwartek: 40 zł/h, piątek–niedziela: 50 zł/h.
                  </p>
                </div>
                ...
                <div className="space-y-3 rounded-xl border p-3">
                  ...
                  <div>
                    ...
                    <input
                      ...
                      placeholder="Zostaw puste, jeśli ma liczyć automatycznie"
        ...
        <section className="mt-6 rounded-2xl bg-white p-4 text-slate-900 shadow md:p-6">
          <div className="mb-4 flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
            <div>
              <h2 className="text-xl font-bold">Archiwum po dacie</h2>
              <p className="text-sm text-slate-500">
                Wybierz dzień, żeby sprawdzić gry i utarg z konkretnej daty.
              </p>
            </div>
          ...
          <div className="mb-4 rounded-xl bg-slate-100 p-4">
            <p className="text-sm text-slate-500">Utarg za wybrany dzień</p>
            <p className="text-2xl font-black">{archiveRevenue.toFixed(2)} zł</p>
          </div>
                ...
                {archiveSessions.map((session) => {
                  ...
                  return (
                    <tr key={session.id} className="border-b">
                      ...
                      <td className="p-2">{session.duration_minutes ?? "-"} min</td>
                      <td className="p-2">{formatMoney(session.hourly_rate)}/h</td>
                      <td className="p-2">
                        {session.member_discount_applied
                          ? `Członek -${MEMBER_DISCOUNT_AMOUNT} zł`
                          : session.discount_name ?? "Brak"}
                ...
                {archiveSessions.length === 0 && (
                  <tr>
                    <td className="p-4 text-slate-500" colSpan={8}>
                      Brak zakończonych gier dla tej daty.
                    </td>
                  </tr>
                )}
              </tbody>
